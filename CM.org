...
Dans linux (post 2.6), on ne fait plus la distinction pour proc et user, qui venait des pressions mémoires.

Tout est dans la structure task, union des deux précédentes.

Dans la structure task, on trouvera :
- La politique d'ordonnancement (OTHER pour les processus normaux, espèce de round robin avec vieillissement, FIFO réservé au temps réel, RR sans vieillissement) : spécifié par la norme POSIX
...
Threads sont les unités considérées par l'ordonnanceur, qui partagent la même espace d'adressage (la structure mm, pour memory management, qui contient entre autres la table des pages).
...
- statique sans vieillissement (pour les processus prioritaires, RR)
- dynamique, selon l'algorithme proche du unix avec decay.

[Bien revoir la correspondance entre stratégies d'ordonnancement et les classes de processus, au fil du temps]
...
En cas de blocage, ou bien au retour en mode U si un flag (runrun) est mis.
...
Si tous les prêts ont un poids de 0, on est à la fin de la période.
...
L'idée est de connaître exactement la taille d'une époque au début de celle-ci.
...
Tic plus bas, alarmes plus précises.
