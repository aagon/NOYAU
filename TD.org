...
Les blocs de données viennent en quatre types :
- Les blocs de données brutes
- Les superblocs : des blocs qui stockent des données sur la partition (début, fin, etc...)
- Les blocs d'inode
- Les blocs d'indirection

Les distinctions entre tous ces types de blocs de données seront vues plus en détail lors du TD sur le système de fichiers.
...
Avant toute chose, le fonctionnement de la structure buf :



On a deux chaînes de blocs doublement chaînés.

b_dev : identifiant du matériel : un entier, vecteur de bits. Bits de poids fort donnent l'indentifiant du driver : code très bas niveau qui va permettre d'utiliser le périphérique, bits de poids faible donnent le numéro de device qu'on veut utiliser avec ce driver.

b_count : nombre de données qui restent à transférer vers ou depuis le device : 0 égale fin d'entrée/sortie.

buffers sont les données du cache (type un peu spécial, à reprendre)

b_blkno : numéro de bloc dans le device
...
Les drivers doivent pouvoir ouvrir 

Une fonction d'entrée/sortie doit avoir un device, et une structure buf *préparée*.
...
Assigne un tampon à un bloc, si le bloc n'a pas déjà un tampon. Et verrouille le bloc.

[ On va refaire un automate ]

av_forw : LRU
av_back : MRU

ligne 260 : on écrase tous les flags, et on met B_BUSY

Pour résumer :

On assure que le buffer rendu est bien placé et verrouillé.

[ automate pris en photo, à reprendre en dot ]
...
#+BEGIN_SRC c
  ...
  void brelse(struct buf *bp)
  {
	  ...
	  if (bfreelist.bflag&B_WANTED) wakeup(&bfreelist);
...
Cette semaine, le fonctionnement du système de fichiers.
...
Le fd, le descripteur de fichier, est l'entier qui est l'index dans la table des fichiers ouverts.

0, 1 et 2 sont réservés à l'entrée standard, la sortie standard et la sortie d'erreur.

La structure file :

Une table globale du système. Table des ouvertures de fichiers.

[à reprendre]



On regarde inode.h, l'implémentation unix v6.


On a une table statique des inodes en train d'être utilisés.

Lien physique vs lien symbolique.

Une inode n'est supprimée que quand la dernière référence (le dernier lien physique) vers cette inode est supprimée : quand i_nlink tombe à 0.


Le lien entre l'inode et le disque :

Organisation du système de fichiers :

Les blocs sont numérotés.

Le premier bloc est le boot bloc.

Ensuite on a un superbloc, qui stocke les métadonnées de l'ensemble des blocs.

Ensuite on a des blocs des inodes.

Puis les blocs de données.
...
La table des inodes en mémoire centrale est un sous-ensemble des inodes stockées sur le disque.

Un répertoire est juste un fichier texte qui stocke des noms de fichiers et les associe à des inodes : c'est donc tout petit, quelques dizaines d'octets.
...
On a bien, dans chacune des structures des fichiers ouverts, une référence vers une entrée *distincte* de la table globale des fichiers ouverts.

Ces deux entrées de la table globale des fichiers ouverts pointent par contre sur la même inode.

Chacun a son propre offset :

Si A écrit tout, puis B
bbac

Si A écrit aaa, puis B, puis re-a :
bbac
