...
La zone OS est toujours mappée au même endroit (elle ne bouge pas dans la mémoire physique, et elle n'est jamais swappée).
...
Remplace la table des pages courante par la table des pages adressée physiquement par le paramètre.
...
On déclare un pointeur (static : enregistré dans le segment de données, mais reste visible uniquement par la fonction) vers une structure proc.
Deux entiers (dans le registre)
Un pointeur vers une structure proc (dans le registre)

On fait pointer p vers le premier élément du tableau (la struct proc du processus 0, le scheduler) : 

On met des registres processeur (le stack pointer) dans la structure u du processus qui a appelé swtch.
...
On se rappelle du p du précédent appel à la fonction, parce qu'on veut recommencer à ce p (pas du début).

On fait compare les priorités, et on garde un pointeur vers le plus prioritaire.

Quand on sort de la boucle après avoir décrémenté i NPROC fois, on a un pointeur p vers la processus à élire.

Est-ce que le processus considéré est à l'état prêt, et est-ce qu'il est en mémoire.
...
Une trappe : Fonction appelée quand on passe du mode U au mode S.
...
Dans les structures rsav, ssav, qsav sont enregistrés des SP qui correspondent à des états différents.
