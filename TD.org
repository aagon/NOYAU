...
La structure callo dans la fichier d'en-tête callo, permet de créer des appels réguliers.

On a un tableau statique de cette structure.

Un define de l'inhibition d'horloge.
...
Celle-ci fait appel au temps global, soit le nombre de secondes depuis 1/1/1970
...
On créé deux entiers externes (que signifie ce mot-clé ??).
...
Si c_time est inférieur à 0 et 
...
Si il y a bien un processus, on incrémente le resident time jusqu'à 127.
...
Ensuite, si pression mémoire, on réveille le swapper.

**** setpri

En gros applique l'algorithme vu en cours, pour recréer un système de round robin.
...
*** Coder une fonction noyau sleep qui ne servirait que de delay
