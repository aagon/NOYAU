...
[ Réponses ]
t = 2 [ refaire le calculs ]
On s'endort sur runin.

On échange le place dans la mémoire entre 103 et 104 toutes les 3 seconde : donc 104 termine son exécution au plus tôt à 17 secondes.

* TD 8 : 04/12/2019

Aujourd'hui, le cache tampon (ou cache d'entrée/sortie)

Cache what ? [ Rappeler une bonne définition générale ]

Mémoire qui entretient un certain rapport avec une mémoire d'intérêt plus grande et plus lente.

Aujourd'hui, toute la mémoire non utilisée est utilisée comme cache tampon.

Cache logiciel : implanté en logiciel [ Bonne définition ]

Utilité : Accélérer les accès aux données de la mémoire d'intérêt.

Algorithme général de fonctionnement du cache :

Est-ce que la donnée est dans le cache, si oui fin.
Sinon, le cache récupère la donnée de la mémoire d'intérêt.
On lit.

Pour l'écriture ?

Comment gérer la cohérence entre cache et mémoire ?
Quelle donnée évincer du cache en cas de remplissage ?

L'écriture suppose aussi la présence d'un bloc du cache, donc suppose éventuellement un accès mémoire pour ramener la mémoire dans le cache, et aussi un autre accès pour répercuter l'écriture.

Concept de localité [ définition à reprendre ]

LRU : On évince la donnée la moins récemment utilisée.
