...
On a à peu près fini le fonctionnement des processus, on se rapproche petit à petit du fonctionnement du disque et .



Le swapper est un processus, le processus d'id 0.

Toujours en mémoire.

S'exécute toujours en mode système.

Priorité maximale : PSWP, la priorité la plus haute. Dès qu'on réveille le swapper, puisque sa priorité d'endormissement est maximale, on déclenche un swtch() à la sortie du mode noyau.

S'endort quand ?

Soit mémoire pleine et rien à swapper : on s'endort sur l'adresse runin.
Mémoire non pleine, et rien à ramener du swap : on s'endort sur l'adresse runout.
Pendant les attentes d'entrée-sortie : on s'endort 

Le in et le out sont, on le comprend, définis par rapport au disque et non par rapport à la mémoire.

On réveille sur runout : quand on veut ramener quelqu'un du swap vers la mémoire centrale.
On réveille sur runin : quand on veut virer quelqu'un de la mémoire centrale, vers le swap.

Aparté :

Autant une écriture peut être asynchrone, autant une lecture ne peut pas l'être. On peut écrire sur le cache tampon, et attendre que le contenu du cache tampon passe sur le disque. Par contre, on ne peut pas vraiment inventer des données.



La fonction sched :

La fonction appelée par le processus swapper. Le processus swapper est toujours dans cette fonction quand il a la main (il peut bien sûr être dans sleep aussi quand il s'endort).
